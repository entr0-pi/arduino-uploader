...
class HelpTabUI:
    ...
    def _build(self):
        ...
        help_text.insert(tk.END, "- Browse to select partitions.csv, mklittlefs, and nvs_partition_gen.py paths\n")
        ...
        help_text.insert(tk.END, "\n")
        help_text.insert(tk.END, "- Save Configuration stores all settings to uploaderGUI.json\n")
        help_text.insert(tk.END, "- Show advanced parameters for baud, LittleFS block/page size, and verify-after-write\n\n")
        help_text.insert(tk.END, "2) Littlefs Upload tab\n")
        help_text.insert(tk.END, "- Add/remove multiple source lines: folder path + target folder + mode (raw/gzip)\n")
        help_text.insert(tk.END, "- Environment status and file preview update live from configured lines\n")
        help_text.insert(tk.END, "- Optional: erase FS partition before flashing\n")
        ...
        help_text.insert(tk.END, "- nvs_keys.h path is configured at the top of this tab\n")
        help_text.insert(tk.END, "- Unlock namespace/key/type/encoding toggle is next to the header path\n")
